...
void addItem(ShoppingList *list)
{
 ...
 list->itemList = (GrocaryItem*)realloc(list->itemList, sizeof(GrocaryItem)*list->length);
    ...
    if (list->itemList != NULL) {
        ...
        fgets(name, 20, stdin);
        for(i=0; name[i] != '\0'; i++)
            list->itemList[list->length-1].productName[i] = name[i];
        
        list->itemList[list->length-1].productName[i-1] = '\0'; // Remove the last character (byte) "enter" stored in the array
        ...
        list->itemList[list->length-1].amount = amount;
        fgetc(stdin);
        ...
        fgets(unit, 10, stdin);
        for(i=0; unit[i] != '\0'; i++)
            list->itemList[list->length-1].unit[i] = unit[i];
        
        list->itemList[list->length-1].unit[i-1] = '\0';
